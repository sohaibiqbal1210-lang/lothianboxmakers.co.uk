...
(function () {
  ...
  /* Machine size cap, given by Lothian Box Makers, 3 September 2026.
     A finished box must satisfy, in millimetres:
       length + width  <= 972
       width  + height <= 980
     Held here in centimetres because the tool works in centimetres. */
  var MAX_LW_CM = 97.2;
  var MAX_WH_CM = 98.0;

  /* Every box can be turned on the machine, so the cap is met if ANY of the
     six orderings of the three sides satisfies both rules. Returns the
     ordering that fits, or null. */
  var ORIENTATIONS = [[0,1,2],[0,2,1],[1,0,2],[1,2,0],[2,0,1],[2,1,0]];

  function fitsCap(dims) {
    for (var i = 0; i < ORIENTATIONS.length; i++) {
      var o = ORIENTATIONS[i];
      var l = dims[o[0]], w = dims[o[1]], h = dims[o[2]];
      if (l + w <= MAX_LW_CM && w + h <= MAX_WH_CM) return [l, w, h];
    }
    return null;
  }

  /* Smallest amount, in cm, that has to come off the box before some
     ordering fits. Used only to word the error. */
  function capOverage(dims) {
    var best = Infinity;
    for (var i = 0; i < ORIENTATIONS.length; i++) {
      var o = ORIENTATIONS[i];
      var l = dims[o[0]], w = dims[o[1]], h = dims[o[2]];
      var over = Math.max(0, l + w - MAX_LW_CM, w + h - MAX_WH_CM);
  ...
  form.addEventListener('submit', function (e) {
    ...
    /* Machine cap. Checked on the internal size, which is the box we make. */
    if (!fitsCap(internal)) {
      return fail('That box is past our machine limit. A finished box must keep ' +
                  'length + width under ' + MAX_LW_CM + 'cm (972mm) and width + height under ' +
                  MAX_WH_CM + 'cm (980mm). With ' + PAD_CM + 'cm of packing room your box comes out ' +
                  fmt(internal) + ', which is about ' + capOverage(internal) + 'cm too big. ' +
                  'Ring 07392 770606 and we will size it with you.');
    }
